            ...
            <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700">
                <div class="p-8">
                    <div class="flex items-center mb-6">
                        ...
                        <div class="ml-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Server Configuration</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Update the DHCP server settings</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{route('updateDhcpServer',[$deviceParam,$server['.id']])}}" enctype="multipart/form-data">
                        ...
                        <div class="space-y-6">
                                ...
                                <select id="interface" name="interface" 
                                    ...
                                    @foreach ($interfaces as $interface)
                                        @if ($interface['type'] != "loopback")
                                            <option {{$server['interface'] == $interface['name'] ? 'selected' : ''}}>
                                                {{$interface['name']}}
                                            </option>
                                        @endif
                                    @endforeach
                            ...
                            <div class="p-4 bg-white dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                ...
                                <input type="text" id="relay" name="relay" value="{{$server['relay'] ?? ''}}"
                            ...
                            <div class="p-4 bg-white dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                ...
                                <input type="text" id="lease-time" name="lease-time" value="{{$server['lease-time']}}"
                            ...
                            <div class="p-4 bg-white dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                ...
                                <select id="authoritative" name="authoritative" 
                                    ...
                                    <option value="after-2s-delay" {{ isset($server['authoritative']) && $server['authoritative'] == "after-2s-delay" ? 'selected' : '' }}>After 2s delay</option>
                                    <option value="after-10s-delay" {{ isset($server['authoritative']) && $server['authoritative'] == "after-10s-delay" ? 'selected' : '' }}>After 10s delay</option>
                                    <option value="no" {{ !isset($server['authoritative']) ? 'selected' : '' }}>No</option>
                                    <option value="yes" {{ isset($server['authoritative']) && $server['authoritative'] == "yes" ? 'selected' : '' }}>Yes</option>
                                </select>
                                @error('authoritative')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="flex flex-wrap gap-4 mt-2">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="always-broadcast" value="true"
                                            {{isset($server['always-broadcast']) ? 'checked' : ''}}
                                    ...
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="add-arp" value="true"
                                            {{isset($server['add-arp']) ? 'checked' : ''}}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-700">
                                        <span class="ml-2 text-sm text-gray-600 dark:text-gray-300">Add ARP for leases</span>
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="use-framed-as-classless" value="true"
                                            {{!isset($server['use-framed-as-classless']) ? 'checked' : ''}}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-700">
                                        <span class="ml-2 text-sm text-gray-600 dark:text-gray-300">Use framed as classless</span>
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="conflict-detection" value="true"
                                            {{!isset($server['conflict-detection']) ? 'checked' : ''}}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:border-gray-700">
